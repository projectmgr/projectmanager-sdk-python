...
from ProjectManager.SDK.models.filedto import FileDto
...
class ProjectFileClient:
    ...
    def upload_project_file(self, projectId: str, filename: str) -> AstroResult[AstroResult[FileDto]]:
        """
        ...
        if result.status_code >= 200 and result.status_code < 300:
            return AstroResult(True, result.status_code, AstroResult.from_json(result.json(), FileDto), None)
        else:
            return AstroResult(False, result.status_code, None, ErrorResult.from_json(result.json()))

    def upload_project_file_to_folder(self, projectId: str, folderId: str, filename: str) -> AstroResult[AstroResult[FileDto]]:
        """
        Uploads a file to a project folder.
