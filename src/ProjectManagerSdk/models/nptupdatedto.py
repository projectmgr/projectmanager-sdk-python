...
class NptUpdateDto:
    ...
    this breaks the Npt out of its recurring series. Sending true or
    omitting it leaves the series intact.
    """

    breakRecurrency: bool | None = None
    """
    Set this to true to break this Npt out of its recurring series as
    part of this update. The Npt is detached from its series, clearing
    its parent/child relationship and its recurrence settings. Any other
    changes in the same update are then applied to this Npt alone rather
    than being propagated across the rest of the series. This has no
    effect if the Npt is not part of a recurring series. An Npt can only
    be made recurring through the NptRecurrency endpoints.
    """
