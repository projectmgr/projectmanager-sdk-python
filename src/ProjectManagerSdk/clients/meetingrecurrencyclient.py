...
class MeetingRecurrencyClient:
    ...
    def create_weekly_recurring_meetings(self, meetingId: str, body: WeeklyRecurringSettingsDto) -> AstroResult[RecurringTaskChangeSetDetails]:
        """
        Changes an existing Meeting into a Recurring Meeting, so that it
        will recur regularly given the specified rules. A Recurring
        Meeting is one that occurs on a specific regular frequency, such
        as Daily, Weekly, Monthly, or Yearly. To create a Recurring
        Meeting, you must first create a regular Meeting with the
        necessary information, then call one of the Create Recurring
        Meeting APIs. To remove an instance of a Recurring Meeting, call
        Delete Recurring Meeting and specify one or more instances of
    ...
    def delete_recurring_meetings(self, meetingId: str, option: str) -> AstroResult[DeletedTasksDto]:
        """
        Removes one or more instances of a Recurring Meeting based on
        the `option` you specify: `this` means to remove a single
        instance, `all` means to remove all instances, or `future` means
        to remove all future instances of the Recurring Meeting. A
        Recurring Meeting is one that occurs on a specific regular
        frequency, such as Daily, Weekly, Monthly, or Yearly. To create
        a Recurring Meeting, you must first create a regular Meeting
        with the necessary information, then call one of the Create
        Recurring Meeting APIs. To remove an instance of a Recurring
        Meeting, call Delete Recurring Meeting and specify one or more
    ...
    def validate_recurring_meeting_settings(self, meetingId: str, body: RecurringTaskSettingsDto) -> AstroResult[RecurringTaskValidationResultDto]:
        """
        Reviews potential updates to a Recurring Meeting and report back
        on the list of changes that would occur if this Recurring
        Meeting was updated with these settings. When making changes to
        a Recurring Meeting, you may want to investigate the
        consequences of your changes first before finalizing the
        changes. You can use the Validate Recurring Meetings API to
        examine these changes. When you are happy with the changes, call
        Update Recurring Meetings to complete them. A Recurring Meeting
        is one that occurs on a specific regular frequency, such as
        Daily, Weekly, Monthly, or Yearly. To create a Recurring
        Meeting, you must first create a regular Meeting with the
        necessary information, then call one of the Create Recurring
        Meeting APIs. To remove an instance of a Recurring Meeting, call
        Delete Recurring Meeting and specify one or more instances of
    ...
    def update_recurring_meeting_settings(self, meetingId: str, body: RecurringTaskSettingsDto) -> AstroResult[RecurringTaskChangeSetDetails]:
        """
        Updates the settings for a Recurring Meeting and regenerates
        occurrences of the Recurring Meeting from the new rules. When
        making changes to a Recurring Meeting, you may want to
        investigate the consequences of your changes first before
        finalizing the changes. You can use the Validate Recurring
        Meetings API to examine these changes. When you are happy with
        the changes, call Update Recurring Meetings to complete them. A
        Recurring Meeting is one that occurs on a specific regular
        frequency, such as Daily, Weekly, Monthly, or Yearly. To create
        a Recurring Meeting, you must first create a regular Meeting
        with the necessary information, then call one of the Create
